...
contract FailingMockAddVerifiedClient {
    ...
    fallback(bytes calldata data) external payable returns (bytes memory) {
        ...
        if (target == VERIFREG_ACTOR_ID) {
            ...
            if (methodNum == ADD_VERIFIED_CLIENT) return _handleAddVerifiedClientFailed();
        }

        if (target == DATACAP_ACTOR_ID) {
            if (methodNum == DATACAP_TRANSFER) return _handleDatacapTransferFailed();
        }

        revert MethodNotFound("FailingMockAddVerifiedClient", methodNum, target);
    }

    function _handleAddVerifiedClientFailed() internal pure returns (bytes memory) {
        return abi.encode(EXIT_CODE_ERROR, 0x00, "");
    }

    function _handleDatacapTransferFailed() internal pure returns (bytes memory) {
        return abi.encode(EXIT_CODE_ERROR, 0x00, "");
    }
